...
class Tank:
    #######################################
    def __init__(self, x=100, y=250, color=(0.4, 0.5, 0.2), cannon_angle=0.0, health=100): #tank Position x,y - Color RGB- Health bar
        self.x = x
        self.y = y
        self.og_x = x
        self.og_y = y
        ...
        self.projectiles = [] 
        self.myBarrier = barrier() #Walls
        self.health = health

    def Tank_Tank_collision(self, other_tank):
        tank_left = self.x - 30 #left
        tank_right = self.x + 30 #right
        tank_top = self.y + 25 #top
        tank_bottom = self.y - 25 #bott
        #for ene
        ...
        if (tank_right >= other_left and tank_left <= other_right and #second cond
    ...
    def draw(self):
        glLoadIdentity() #with every action we need to load the identity
    ...
    def draw_tank_body(self):
        ...
        glColor3d(0, 0, 0)
        glutSolidCylinder(18, 1, 5, 1)  # Circle 
    ...
    def draw_cannon(self):
        ...
        self.draw_quad(0, -2.5, self.cannon_length, 2.5)  # Cannon #0-2.5-35-2.5
